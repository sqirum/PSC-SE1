 ...
 *	Utilização:
 *
 *	$ gcc mini_snprintf_test.c mini_snprintf.c ../exercicio1/int_to_string.c ../exercicio2/float_to_string.c -o mini_snprint
 *
 *	$ ./mini_snprintf
 */
...
struct {
	...
	{.buffer_size = 13, .format = "aa %x bb %s", .values = {{10}, {.s = "YY"}}, .result_size = 12, .result_string = "aa 0xa bb YY"},
	{.buffer_size = 4, .format = "%d %c", .values = {{2}, {'a'}}, .result_size = 3, .result_string = "2 a"},
...
int main() {
	...
	for (i = 0; i < ARRAY_SIZE(test_array) - 2; ++i) {
		result = mini_snprintf(test_array[i].buffer,
